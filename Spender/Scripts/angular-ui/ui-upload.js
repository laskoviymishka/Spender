...
!function(a, b) { return "function" == typeof define && define.amd ? void define("angular-file-upload", ["angular"], function(a) { return b(a); }) : b(a); }(angular || null, function(a) {
    var b = a.module("angularFileUpload", []);
    return b.directive("ngFileDrop", [
        "$fileUploader", function(b) {
            "use strict";
            return {
                link: b.isHTML5 ? function(a, b, c) {
                    ...
                    }).bind("dragleave", function(c) { c.target === b[0] && a.$broadcast("file:removeoverclass"); });
                } : a.noop
            };
        }
    ]), b.directive("ngFileOver", function() {
        "use strict";
        return { link: function(a, b, c) { a.$on("file:addoverclass", function() { b.addClass(c.ngFileOver || "ng-file-over"); }), a.$on("file:removeoverclass", function() { b.removeClass(c.ngFileOver || "ng-file-over"); }); } };
    }), b.directive("ngFileSelect", [
        "$fileUploader", function(a) {
            "use strict";
            return {
                link: function(b, c, d) {
        ...
        "$compile", "$rootScope", "$http", "$window", function(b, c, d, e) {
            ...
            return f.prototype = {
                constructor: f,
                _emptyFileFilter: function(b) { return a.isElement(b) ? !0 : !!b.size; },
                _queueLimitFilter: function() { return this.queue.length < this.queueLimit; },
                bind: function(a, b) { return this.scope.$on(this._timestamp + ":" + a, b.bind(this)); },
                trigger: function(a) { arguments[0] = this._timestamp + ":" + a, this.scope.$broadcast.apply(this.scope, arguments); },
                isHTML5: !(!e.File || !e.FormData),
                addToQueue: function(b, c) {
                    var d = this.queue.length, e = "length" in b ? b : [b];
                    a.forEach(e, function(b) {
                        var d = this.filters.length ? this.filters.every(function(a) { return a.call(this, b); }, this) : !0, e = new g(a.extend({ url: this.url, alias: this.alias, headers: a.copy(this.headers), formData: a.copy(this.formData), removeAfterUpload: this.removeAfterUpload, withCredentials: this.withCredentials, method: this.method, uploader: this, file: b }, c));
                ...
                clearQueue: function() { for (; this.queue.length;) this.queue[this.queue.length - 1].remove(); },
                ...
                cancelItem: function(a) {
                    ...
                    c.isUploading && c[d] && c[d].abort();
                },
                uploadAll: function() {
                    var a = this.getNotUploadedItems().filter(function(a) { return !a.isUploading; });
                    a.forEach(function(a) { a.index = a.index || this._nextIndex++, a.isReady = !0; }, this), a.length && this.uploadItem(a[0]);
                },
                cancelAll: function() { this.getNotUploadedItems().forEach(function(a) { this.cancelItem(a); }, this); },
                getIndexOfItem: function(b) { return a.isNumber(b) ? b : this.queue.indexOf(b); },
                getNotUploadedItems: function() { return this.queue.filter(function(a) { return !a.isUploaded; }); },
                getReadyItems: function() { return this.queue.filter(function(a) { return a.isReady && !a.isUploading; }).sort(function(a, b) { return a.index - b.index; }); },
                _render: function() { this.scope.$$phase || this.scope.$digest(); },
                ...
                _complete: function() {
                    ...
                    return this.isUploading = !1, a.isDefined(b) ? void this.uploadItem(b) : (this.trigger("completeall", this.queue), this.progress = this._getTotalProgress(), void this._render());
                },
                _xhrTransport: function(b) {
                    var c = b._xhr = new XMLHttpRequest, d = new FormData, e = this;
                    this.trigger("beforeupload", b), b.formData.forEach(function(b) { a.forEach(b, function(a, b) { d.append(b, a); }); }), d.append(b.alias, b.file), c.upload.onprogress = function(a) {
                    ...
                    }, c.onerror = function() { e.trigger("in:error", c, b), e.trigger("in:complete", c, b); }, c.onabort = function() { e.trigger("in:cancel", c, b), e.trigger("in:complete", c, b); }, c.open(b.method, b.url, !0), c.withCredentials = b.withCredentials, a.forEach(b.headers, function(a, b) { c.setRequestHeader(b, a); }), c.send(d);
                },
                _iframeTransport: function(b) {
                    ...
                    b._form && b._form.replaceWith(e), b._form = c, this.trigger("beforeupload", b), e.prop("name", b.alias), b.formData.forEach(function(b) { a.forEach(b, function(b, d) { c.append(a.element('<input type="hidden" name="' + d + '" value="' + b + '" />')); }); }), c.prop({ action: b.url, method: "POST", target: d.prop("name"), enctype: "multipart/form-data", encoding: "multipart/form-data" }), d.bind("load", function() {
                ...
                _isSuccessCode: function(a) { return a >= 200 && 300 > a || 304 === a; },
                _transformResponse: function(a) { return d.defaults.transformResponse.forEach(function(b) { a = b(a); }), a; }
